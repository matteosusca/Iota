...
const isAppLoading = computed(() => !authStore.isInitialized);
...
<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col">
    <Transition name="fade" mode="out-in">
      <!-- Splash Screen -->
      <div v-if="isAppLoading" key="splash" class="fixed inset-0 z-[10000] flex flex-col items-center justify-center bg-slate-950">
        <div class="relative mb-6">
          <div class="w-16 h-16 border-4 border-slate-800 border-t-green-500 rounded-full animate-spin"></div>
          <div class="absolute inset-0 flex items-center justify-center text-green-500 text-2xl font-black">ι</div>
        </div>
        <h1 class="text-3xl font-light tracking-widest animate-pulse">IOTA</h1>
        <p class="text-slate-500 text-xs mt-4 uppercase tracking-[0.3em] font-semibold">Synchronizing...</p>
      </div>

      <!-- Main Application Content -->
      <div v-else key="content" class="flex flex-col flex-grow">
        <!-- Main Content Area -->
