# KaizenFit Production Deployment Guide

This guide explains how to deploy the full KaizenFit application stack into a production environment, specifically targeting **Portainer** and **Nginx Proxy Manager**.
...
|----------------------|---------------|-------------|
| `DB_USER`            | `kaizen`      | Username for Postgres (optional default provided) |
| `DB_PASSWORD`        | `securepassword`| Strong password for your DB! |
| `DB_NAME`            | `kaizenfit`   | Target database name |
| `CORS_ORIGIN`        | `https://kaizenfit.example.com` | Ensure this matches your final domain. |
...
3. **Domain Names**: Enter your desired front-facing domain (e.g., `kaizenfit.example.com`).
4. **Scheme**: `http`
...
- **API Errors / Auth Fails**: Verify `CORS_ORIGIN` in portainer exactly matches the `https://kaizenfit.example.com` origin you defined. Don't leave trailing slashes!
...
You're done! Enjoy your self-hosted version of KaizenFit!
